  ...
  validate(formState, updatedKeys = null) {
    const keys = updatedKeys || Object.keys(formState),
      results = updatedKeys
        ? {
            valid:
              typeof formState.valid !== 'undefined' ? formState.valid : true,
...
function FormProvider({ initFormState, children }) {
  const validator = useMemo(() => new Validator(), []),
    ...
    dangerouslyUpdateFormState = useCallback(
      (updates) => {
        setFormState({ ...formState, ...updates })
      },
      [formState, setFormState]
    ),
    validateFormState = useCallback(
      (onSuccess) => {
...
FormProvider.propTypes = {
  initFormState: PropTypes.func,
  children: PropTypes.node,
}

export function withFormContext(Component, initFormState) {
  return <FormProvider initFormState={initFormState}>{Component}</FormProvider>
}
