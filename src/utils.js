...
function generateBasicScheduleDetails(settings) {
  ...
  switch (frequency) {
    ...
    case 'monthly':
      periodLabel = `every month on ${dayToLabel(
        dayOfWeek
      )} of week ${weekOfMonth}`
      break
  }
  ...
  const amPmLabel = amPm === 'am' ? 'AM' : 'PM',
    ...
    hourLabel =
      hour === -1 ? 'every hour' : `the hour ${hour} ${amPmLabel} ${tz}`,
...
function generateShortBasicScheduleDetails(settings) {
  ...
  const amPmLabel = amPm === 'am' ? ' AM' : ' PM',
    ...
    minuteLabel =
      minute === -1 ? '*' : `${pad(minute, 2)}${hour !== -1 ? amPmLabel : ''}`,
    tzLabel = hour !== -1 ? ` ${tz}` : ''
...
export function resolvePublishConfig(metaPublishConfigs, publishConfig) {
  if (publishConfig.ref) {
    const realPublishConfig = metaPublishConfigs?.publishConfigs.find(
...
export function resolveChannel(metaChannels, channel) {
  if (channel.ref) {
    const realChannel = metaChannels?.channels.find((c) => c.id === channel.ref)
