...
function ScheduleBuilder({ open, onSubmit, onClose }) {
  const { formState, updateFormState, validateFormState } =
      useContext(FormContext),
    { mode, expr, validations } = formState,
    submit = useCallback(
      (formState) => {
        const { mode, expr, model, ...rest } = formState
  ...
  return (
    <Modal hidden={!open} onClose={handleClose}>
      <Form className="schedule-builder">
        ...
        >
          <StackItem>
            <SegmentedControl value={mode} onChange={handleChangeMode}>
              <SegmentedControlItem
          ...
          <StackItem>
            {mode === 'basic' && <BasicScheduleForm />}
            {mode === 'manual' && (
              <Validation name="expr" validation={cronExprIsValid}>
                <TextField
                  placeholder={
...
export default function ScheduleBuilderWrapper({
  ...
  const initFormState = useCallback(() => {
    ...
    if (schedule) {
      ;(initialState.expr = schedule), (initialState.model = parse(schedule))
    }
  ...
  return withFormContext(
    <ScheduleBuilder schedule={schedule} settings={settings} {...rest} />,
    initFormState
  )
}
