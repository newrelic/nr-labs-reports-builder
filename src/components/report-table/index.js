...
function ReportLastRunTableRowCell({ report }) {
  const {
      params: { accountId },
    } = useContext(RouteContext),
    { loading, data, error } = useNrqlQuery({
      accountId: accountId,
  ...
  return (
    <>
      ...
      {lastRunDate && <span>{formatDateTimeForMillis(lastRunDate)}</span>}
    </>
  )
}

ReportLastRunTableRowCell.propTypes = {
...
export default function ReportTable({
    ...
    renderRow = useCallback(
      ({ item: report, index }) => (
        <TableRow key={report.id} actions={getActions}>
          <TableRowCell>
            <Link onClick={() => onSelectReport(index)}>{report.name}</Link>
          </TableRowCell>
  ...
  return (
    <>
      <Table className="report-table" items={reports}>
        <TableHeader>
