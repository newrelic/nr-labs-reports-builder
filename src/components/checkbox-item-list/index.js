...
import { Checkbox, List, ListItem, Spinner } from 'nr1'
...
function CheckboxItemList(
  ...
  const pickRef = useRef(null),
    ...
    handleSelectItem = useCallback(
      (e) => {
        ...
        if (selected && index === -1) {
          setCheckedItems([
            ...checkedItems,
            [target.value, target.dataset.itemName],
          ])
          return
        }
    ...
    View = useMemo(() => {
      ...
      if (error) {
        // @TODO
        return <ErrorView heading="oops" description="oops2" error={error} />
      }
...
const CheckboxItemListWrapper = forwardRef(CheckboxItemList)

export default CheckboxItemListWrapper
