...
export default function MultiSelect2({
  ...
  return (
    <div className={`multiselect ${className || ''}`} ref={thisComponentRef}>
      ...
      {invalid && <div className="errorMessage">{invalid}</div>}
      {showItemsList ? (
        <div className="list" style={{ width: itemsListWidth }}>
          {items.map((item, i) => (
            <div className="item" style={{ width: checkboxWidth }} key={i}>
              <input
                type="checkbox"
                value={item.value}
                id={item.value}
                checked={
                  value.length &&
                  value.findIndex((v) => v.value === item.value) !== -1
                }
