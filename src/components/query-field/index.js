...
import { accountIdsNotEmptyValidNumbers, queryNotEmpty } from '../validations'
import { FormContext, Validation } from '../../contexts/form'
import { UI_CONTENT } from '../../constants'

export default function QueryField() {
  const { formState, dangerouslyUpdateFormState, updateFormState } =
      useContext(FormContext),
    { accountIds, accounts } = formState,
    skip = formState.accounts ? true : false,
    { loading, data, error } = useAccountsQuery({
      skip,
    }),
    handleChangeAccounts = useCallback(
      (items) => {
    ...
    accountMenuLabeler = useCallback(
      (item, index, items) =>
  ...
  return (
    <div className="query-field">
      <CustomField label={UI_CONTENT.QUERY_FORM.FIELD_LABEL_ACCOUNTS}>
        <Validation
          name="accountIds"
              ...
              {accounts && (
                <StackItem className="accounts-multiselect">
                  <MultiSelect2
                    ...
                    placeholderText={
                      UI_CONTENT.QUERY_FORM.ACCOUNTS_FIELD_PLACEHOLDER
                    }
      ...
      <CustomField label={UI_CONTENT.QUERY_FORM.FIELD_LABEL_QUERY}>
        <Validation name="query" validation={queryNotEmpty}>
          {loading ? (
            <Spinner />
          ) : (
            <Grid>
