...
import { Form, Icon, Select, SelectItem, Stack, StackItem } from 'nr1'
...
export default function BasicScheduleForm() {
  const { formState, updateFormState } = useContext(FormContext),
    ...
    { options, parseTimezone } = useTimezoneSelect({
      labelStyle: 'original',
      timezones: TIMEZONES,
    }),
    ...
    handleChangeFrequency = useCallback(
      (_, v) => {
        update({
          frequency: v,
    ...
    handleChangePeriod = useCallback(
      (_, v) => {
        update({ period: v })
      },
      [update]
    ),
    handleChangeDaysOfWeek = useCallback(
      (items) => {
        update({ daysOfWeek: items })
      },
      [update]
    ),
    handleChangeDayOfWeek = useCallback(
      (_, v) => {
        update({ dayOfWeek: v })
      },
      [update]
    ),
    handleChangeHour = useCallback(
      (_, v) => {
        update({ hour: v })
      },
      [update]
    ),
    handleChangeMinute = useCallback(
      (_, v) => {
        update({ minute: v })
      },
      [update]
    ),
    handleChangeAmPm = useCallback(
      (_, v) => {
        update({ amPm: v })
      },
      [update]
    ),
    handleChangeTimezone = useCallback(
      (_, v) => {
    ...
    handleChangeWeekOfMonth = useCallback(
      (_, v) => {
        update({ weekOfMonth: v })
      },
      [update]
    ),
    dayOfWeekMenuLabeler = useCallback(
      (item, index, items) =>
  ...
  return (
    ...
    >
      ...
      >
        <SelectItem
          value={UI_CONTENT.BASIC_SCHEDULE_FORM.FREQUENCY_VALUE_DAILY}
        >
          {UI_CONTENT.BASIC_SCHEDULE_FORM.FREQUENCY_LABEL_DAILY}
        </SelectItem>
        <SelectItem
          value={UI_CONTENT.BASIC_SCHEDULE_FORM.FREQUENCY_VALUE_WEEKLY}
        >
          {UI_CONTENT.BASIC_SCHEDULE_FORM.FREQUENCY_LABEL_WEEKLY}
        </SelectItem>
        <SelectItem
          value={UI_CONTENT.BASIC_SCHEDULE_FORM.FREQUENCY_VALUE_MONTHLY}
        >
      ...
      {frequency === 'daily' && (
        ...
        >
          <SelectItem
            value={UI_CONTENT.BASIC_SCHEDULE_FORM.PERIOD_VALUE_EVERYDAY}
          >
            {UI_CONTENT.BASIC_SCHEDULE_FORM.PERIOD_LABEL_DAY}
          </SelectItem>
          <SelectItem
            value={UI_CONTENT.BASIC_SCHEDULE_FORM.PERIOD_VALUE_WEEKDAYS}
          >
            {UI_CONTENT.BASIC_SCHEDULE_FORM.PERIOD_LABEL_WEEKDAYS}
          </SelectItem>
          <SelectItem
            value={UI_CONTENT.BASIC_SCHEDULE_FORM.PERIOD_VALUE_WEEKENDS}
          >
      ...
      {frequency === 'weekly' && (
        <CustomField label={UI_CONTENT.BASIC_SCHEDULE_FORM.FIELD_LABEL_PERIOD}>
          <MultiSelect2
            showChips={false}
            items={DAYS_OF_WEEK}
            value={daysOfWeek}
            ...
            placeholderText={
              UI_CONTENT.BASIC_SCHEDULE_FORM.DAYS_OF_WEEK_FIELD_PLACEHOLDER
            }
        ...
        >
          <StackItem>
            ...
            >
              {DAYS_OF_WEEK.map((item, index) => (
                <SelectItem key={`day-of-week-${index}`} value={item.value}>
                  {item.label}
                </SelectItem>
              ))}
            </Select>
          ...
          <StackItem>
            <Select
              ...
              value={weekOfMonth}
            >
              {WEEKS_OF_MONTH.map((item, index) => (
                <SelectItem key={`week-of-month-${index}`} value={item.value}>
                  {item.label}
                </SelectItem>
              ))}
            </Select>
      ...
      >
        <div className="time-select">
          <Select label="" onChange={handleChangeHour} value={hour}>
            {HOURS.map((hour) => (
              <SelectItem key={hour} value={hour}>
                {hour === -1 ? 'Every hour' : pad(hour, 2)}
              </SelectItem>
            ))}
          </Select>
          <div className="colon">:</div>
          <Select label="" onChange={handleChangeMinute} value={minute}>
            {MINUTES.map((minute) => (
              <SelectItem key={minute} value={minute}>
                {minute === -1 ? 'Every minute' : pad(minute, 2)}
              </SelectItem>
            ))}
          </Select>
          <Select
