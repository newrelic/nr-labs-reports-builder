...
import { Button, Card, CardHeader, CardBody, Icon, Tabs, TabItem } from 'nr1'
...
function ChannelsView() {
  const { navigate } = useContext(RouteDispatchContext),
    ...
    handleEditChannel = useCallback(
      (index) => {
        navigate(ROUTES.EDIT_CHANNEL, { selectedChannel: index })
      },
      [navigate]
    ),
    handleDeleteChannel = useCallback(
      (index) => {
        if (!confirm(UI_CONTENT.HOME.CHANNELS.DELETE_CHANNEL_PROMPT)) {
          return
        }
  ...
  if (!metaChannels || metaChannels.channels.length === 0) {
    return (
      <EmptyView
...
function PublishConfigurationsView() {
  const { navigate } = useContext(RouteDispatchContext),
    ...
    handleEditConfig = useCallback(
      (index) => {
        navigate(ROUTES.EDIT_SCHEDULE, { selectedConfig: index })
      },
      [navigate]
    ),
    handleDeleteConfig = useCallback(
      (index) => {
        if (!confirm(UI_CONTENT.HOME.SCHEDULES.DELETE_SCHEDULE_PROMPT)) {
          return
        }
  ...
  if (!metaPublishConfigs || metaPublishConfigs.publishConfigs.length === 0) {
    return (
      <EmptyView
...
function ReportsView() {
  const { navigate } = useContext(RouteDispatchContext),
    ...
    handleEditReport = useCallback(
      (index) => {
        navigate(ROUTES.EDIT_REPORT, { selectedReportIndex: index })
      },
      [navigate]
    ),
    handleDeleteReport = useCallback(
      (index) => {
  ...
  return (
    <div className="home-screen">
      <Tabs defaultValue={tab || 'reports'}>
        <TabItem value="reports" label={UI_CONTENT.HOME.TAB_LABEL_REPORTS}>
          <ReportsView />
        </TabItem>
        <TabItem
        ...
        <TabItem value="channels" label={UI_CONTENT.HOME.TAB_LABEL_CHANNELS}>
          <ChannelsView />
        </TabItem>
      </Tabs>
    </div>
