...
function publishConfigFromFormState(formState) {
  const publishConfig =
    formState.mode ===
    UI_CONTENT.EDIT_SCHEDULE_FORM.MODE_VALUE_USE_EXISTING_SCHEDULE
      ? { ref: formState.ref }
      : {
          id: formState.id,
          name: formState.name,
...
function buildParentFormState(formState, selectedConfig) {
  ...
  if (selectedConfig >= 0) {
    ...
    publishConfigs = [...formState.prevConfigs, publishConfig]
  }
          ...
          <Card>
            <CardHeader
              ...
              subtitle={
                formState.references > 1
                  ? UI_CONTENT.EDIT_SCHEDULE_SCREEN.EDIT_SHARED_SCHEDULE_MESSAGE(
            ...
            <CardBody>
              ...
              >
                ...
                >
                  <StackItem grow>
                    <PublishConfigForm selectedConfig={selectedConfig} />
                  </StackItem>
...
EditPublishConfigScreen.propTypes = {
  ...
  useEffect(() => {
    if (writeError) {
      Toast.showToast({
        ...
        description: UI_CONTENT.EDIT_SCHEDULE_FORM.SAVE_ERROR_DESCRIPTION(
          writeError.message
        ),
        /*
        @TODO
    ...
    if (writeFinished) {
      Toast.showToast({
        ...
        description: UI_CONTENT.EDIT_SCHEDULE_FORM.SAVE_SUCCESS_DESCRIPTION(
          formState.name
        ),
...
export default function EditPublishConfigScreenWrapper(props) {
  const {
      params: { formState, selectedConfig },
    } = useContext(RouteContext),
    { manifest, publishConfigs: metaPublishConfigs } =
  ...
  if (!formState || formState.parentFormState === null) {
    ...
    return withFormContext(
      <StandaloneEditPublishConfigScreen {...props} />,
      initFormState
    )
  }
  ...
  return withFormContext(
    <ReportEditPublishConfigScreen {...props} />,
    initFormState
  )
}
