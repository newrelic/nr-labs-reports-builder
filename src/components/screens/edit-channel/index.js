...
import { ROUTES, SYMBOLS, UI_CONTENT } from '../../../constants'
...
function formStateFromChannel(
  ...
  const channel =
      selectedChannel >= 0
        ? resolveChannel(metaChannels, channels[selectedChannel])
        : newChannel(),
  ...
  if (channel.type === SYMBOLS.CHANNEL_TYPES.EMAIL) {
    ...
    ;(formState.emailAttachOutput =
      typeof channel.attachOutput !== 'undefined'
        ...
        : true),
      (formState.emailPassThrough =
        typeof channel.passThrough !== 'undefined' ? channel.passThrough : true)
    formState.emailSubject = channel.subject
    formState.emailTo = channel.to
      ? channel.to.replaceAll(/\s*,\s*/gu, '\n')
      : ''
    formState.emailCc = channel.cc
      ? channel.cc.replaceAll(/\s*,\s*/gu, ',')
      : ''
...
function channelFromFormState(formState) {
  if (
    formState.mode ===
    UI_CONTENT.EDIT_CHANNEL_FORM.MODE_VALUE_USE_EXISTING_CHANNEL
  ) {
  ...
  if (formState.type === SYMBOLS.CHANNEL_TYPES.EMAIL) {
    ...
    channel.to = formState.emailTo
      ? formState.emailTo.replaceAll(/\s*\n+\s*/gu, ',')
      : ''
    channel.cc = formState.emailCc
      ? formState.emailCc.replaceAll(/\s*\n+\s*/gu, ',')
      : ''
    channel.body = formState.emailBody || ''
    channel.queryResultsHtmlMaxRows = Number(
      formState.emailQueryResultsHtmlMaxRows
    )
...
function buildParentFormState(formState, selectedChannel) {
  ...
  if (selectedChannel >= 0) {
    ...
    channels = [...formState.prevChannels, channel]
  }
...
function EditChannelScreen({
  ...
  const { formState, validateFormState } = useContext(FormContext),
    { writing } = useContext(StorageContext),
        ...
        <LayoutItem>
          <Card>
            <CardHeader
              ...
              subtitle={
                formState.references > 1
                  ? UI_CONTENT.EDIT_CHANNEL_SCREEN.EDIT_SHARED_CHANNEL_MESSAGE(
            ...
            <CardBody>
              ...
              >
                ...
                >
                  <StackItem grow>
                    <ChannelForm selectedChannel={selectedChannel} />
                  </StackItem>
...
function StandaloneEditChannelScreen(props) {
  const {
    ...
    { formState, dangerouslyUpdateFormState } = useContext(FormContext),
    { navigate, home } = useContext(RouteDispatchContext),
    { writeError, writeFinished } = useContext(StorageContext),
    { updateChannel } = useManifestWriter(),
    writeFormState = useCallback(
      (formState) => {
        updateChannel(channelFromFormState(formState), selectedChannel)
      },
  ...
  useEffect(() => {
    if (writeError) {
      Toast.showToast({
        ...
        description: UI_CONTENT.EDIT_CHANNEL_FORM.SAVE_ERROR_DESCRIPTION(
          writeError.message
        ),
        /*
        @TODO
    ...
    if (writeFinished) {
      Toast.showToast({
        ...
        description: UI_CONTENT.EDIT_CHANNEL_FORM.SAVE_SUCCESS_DESCRIPTION(
          formState.name
        ),
...
function ReportEditChannelScreen(props) {
  const {
    ...
    { formState } = useContext(FormContext),
...
export default function EditChannelScreenWrapper(props) {
  const {
      params: { formState, selectedChannel },
    } = useContext(RouteContext),
    {
      manifest,
  ...
  if (!formState) {
    ...
    return withFormContext(
      <StandaloneEditChannelScreen {...props} />,
      initFormState
    )
  }
  ...
  return withFormContext(
    <ReportEditChannelScreen {...props} selectedChannel={selectedChannel} />,
    initFormState
  )
}
