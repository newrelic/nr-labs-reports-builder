...
export default function PublishConfigsTable({
  ...
  const getActions = useMemo(() => {
    ...
    renderRow = useCallback(
      ({ item, index }) => {
        const publishConfig = resolvePublishConfig(metaPublishConfigs, item),
          settings =
            publishConfig.metadata && publishConfig.metadata['schedule-builder']

        return (
          <TableRow key={publishConfig.id} actions={getActions}>
            <TableRowCell>
              <Link onClick={() => onEditConfig(index)}>
                {publishConfig.name}
              </Link>
            </TableRowCell>
            <TableRowCell>
              {generateShortScheduleDetails(publishConfig.schedule, settings)}
            </TableRowCell>
            <TableRowCell>
  ...
  return (
    <Table className="publish-configs-table" items={publishConfigs}>
      <TableHeader>
...
PublishConfigsTable.propTypes = {
  publishConfigs: PropTypes.array,
