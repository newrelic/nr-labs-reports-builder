...
function rebuildChannels(newPublishConfig, newChannels) {
  ...
  publishConfigChannels.forEach((channel, index) => {
    ...
    if (channel.ref) {
      const jindex = newChannels.find((c) => c.id === channel.ref)
...
function rebuildMetaPublishConfigs(
  ...
  for (
    ...
    for (let jindex = indices.length - 1; jindex >= 0; jindex -= 1) {
      newMetaPublishConfigs.publishConfigs[index].channels.splice(
        indices[jindex],
        1
      )
    }
...
function buildRealPublishConfig(
  ...
  const newPublishConfig = clone(
      resolvePublishConfig(metaPublishConfigs, publishConfig)
    ),
  ...
  newPublishConfig.channels = publishConfigChannels
    ? publishConfigChannels.map((c) => clone(resolveChannel(metaChannels, c)))
    : []
...
export default function useManifestWriter() {
  const {
    ...
    update = useCallback(
      (report, reportIndex = -1) => {
        if (
          ...
          throw new Error(
            `Invalid report index ${reportIndex} found during update`
          )
        }
    ...
    updatePublishConfig = useCallback(
      (publishConfig, publishConfigIndex = -1) => {
        if (
          ...
          throw new Error(
            `Invalid publish config index ${publishConfigIndex} found during update`
          )
        }
        ...
        const newMetaManifest = metaManifest
          ...
          updatedChannelIds = rebuildChannels(
            newPublishConfig,
        ...
        newMetaPublishConfigs.publishConfigs.splice(
          publishConfigIndex >= 0
            ? publishConfigIndex
            : newMetaPublishConfigs.publishConfigs.length,
        ...
        if (
          publishConfigIndex >= 0 ||
          (updatedChannelIds && updatedChannelIds.length > 0)
        ) {
          touchReferrerReports(
            newMetaManifest,
            newMetaPublishConfigs,
    ...
    updateChannel = useCallback(
      (channel, channelIndex = -1) => {
        if (
          channelIndex >= 0 &&
          (!metaChannels || channelIndex >= metaChannels.channels.length)
        ) {
          throw new Error(
            `Invalid channel index ${channelIndex} found during update`
          )
        }
    ...
    createPublishConfig = useCallback(
      (publishConfig) => {
      ...
      (reportIndices) => {
        ...
        for (let index = reportIndices.length - 1; index >= 0; index -= 1) {
          ...
          if (reportIndex >= newMetaManifest.reports.length) {
            throw new Error(
              `Invalid report index ${reportIndex} found during delete`
            )
          }
    ...
    deletePublishConfigs = useCallback(
      (publishConfigIndices) => {
        ...
        for (
          let index = publishConfigIndices.length - 1;
          index >= 0;
          index -= 1
        ) {
          ...
          if (
            publishConfigIndex >= newMetaPublishConfigs.publishConfigs.length
          ) {
            throw new Error(
              `Invalid publish config index ${publishConfigIndex} found during delete`
            )
          }
    ...
    deleteChannels = useCallback(
      (channelIndices) => {
        ...
        for (let index = channelIndices.length - 1; index >= 0; index -= 1) {
          const channelIndex = channelIndices[index]

          if (channelIndex >= newMetaChannels.channels.length) {
            throw new Error(
              `Invalid channel index ${channelIndex} found during delete`
            )
          }
          ...
          channelIds.push(newMetaChannels.channels[channelIndex].id)
