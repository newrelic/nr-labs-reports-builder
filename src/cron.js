...
const DAY_OF_WEEK_SYMS = DAYS_OF_WEEK.reduce(
  (a, v) => ((a[v] = Symbol.for(v)), a),
  {}
)
...
function deserialize(val, range, values = [], allowIncrement = true) {
  if (val === ALL_WILDCARD) {
  ...
  if (allowIncrement && m[5]) {
    return {
      x:
        m[6] === ALL_WILDCARD ? ALL_WILDCARD_SYM : toNumberInRange(m[6], range),
      y: toNumber(m[7]),
...
function serialize(val, range, values = [], allowIncrement = true) {
  ...
  if (valType === 'object' && isDefined(val.x)) {
    ...
    switch (w) {
      case RANGE_WILDCARD_SYM:
        ...
        return `${checkAndGetNumberInRange(
          x,
          range
        )}-${checkAndGetNumberInRange(y, range)}`
      ...
      case INCREMENT_WILDCARD_SYM:
        if (allowIncrement && isDefined(y)) {
          const dx =
              x === ALL_WILDCARD_SYM
                ? ALL_WILDCARD
                : checkAndGetNumberInRange(x, range),
...
function parseMinutes(cron) {
  return copy(
    cron,
    MINUTES_FIELD,
    deserialize(cron[MINUTES_FIELD], RANGE_MINUTES)
  )
}
...
function minutesToString(cron) {
  return copy(
    cron,
    MINUTES_FIELD,
    serialize(cron[MINUTES_FIELD], RANGE_MINUTES)
  )
}
  ...
  if (typeof val === 'object' && isDefined(val.x)) {
    ...
    if (w === WEEKDAY_WILDCARD_SYM) {
      return copy(
        cron,
        DAYS_OF_MONTH_FIELD,
        `${checkAndGetNumberInRange(x, RANGE_DAYS_OF_MONTH)}${WEEKDAY_WILDCARD}`
      )
    }
  }
...
function parseMonths(cron) {
  return copy(
    cron,
    MONTHS_FIELD,
    deserialize(cron[MONTHS_FIELD], RANGE_MONTHS, MONTHS)
  )
}
...
function monthsToString(cron) {
  return copy(
    cron,
    MONTHS_FIELD,
    serialize(cron[MONTHS_FIELD], RANGE_MONTHS, MONTH_SYMS)
  )
}
...
function parseDaysOfWeek(cron) {
  ...
  return copy(
    cron,
    DAYS_OF_WEEK_FIELD,
    deserialize(val, RANGE_DAYS_OF_WEEK, DAY_OF_WEEK_SYMS, false)
  )
}
...
function daysOfWeekToString(cron) {
  ...
  if (typeof val === 'object' && isDefined(val.x)) {
    ...
    if (w === INSTANCE_WILDCARD_SYM) {
      return copy(
        cron,
        DAYS_OF_WEEK_FIELD,
        `${checkAndGetNumberInRange(
          x,
          RANGE_DAYS_OF_WEEK
        )}${INSTANCE_WILDCARD}${checkAndGetNumberInRange(
          y,
          RANGE_WEEKS_IN_MONTH
        )}`
      )
    }
  }
  ...
  return copy(
    cron,
    DAYS_OF_WEEK_FIELD,
    serialize(val, RANGE_DAYS_OF_WEEK, DAYS_OF_WEEK, false)
  )
}
...
function parse(s) {
  ...
  return parseYears(
    parseDaysOfWeek(
      parseMonths(parseDaysOfMonth(parseHours(parseMinutes(vals))))
    )
  )
}

function stringify(minutes, hours, daysOfMonth, months, daysOfWeek, years) {
  const c = cron([
  ...
  return yearsToString(
    daysOfWeekToString(
      monthsToString(daysOfMonthToString(hoursToString(minutesToString(c))))
    )
  ).join(' ')
}
